...
class SearchService:
    ...
    async def _cache_set(self, key: str, value: Any, ttl: int = None):
        ...
        if redis_conn:
            try:
                await redis_conn.setex(key, ttl or self._cache_ttl, ujson.dumps(value))
            except Exception as e:
    ...
    async def search_inventory_for_customer(
        ...
        chunk_size = 100
        chunks = [
            base_inventory[i : i + chunk_size] for i in range(0, len(base_inventory), chunk_size)
        ]
    ...
    def _sort_and_categorize(
        self, inventory: List[Dict], sort_by: str, sort_order: str, limit: int, customer: Dict
    ) -> Dict[str, Any]:
    ...
    async def live_inventory_search(
        self, customer_id: str, search_term: str, configuration: Dict[str, Any], limit: int = 20
    ) -> Dict[str, Any]:
        """
        ...
        quick_results = await loop.run_in_executor(
            self._executor, self._quick_search, search_term, limit * 2  # Get more for NPV filtering
        )
        ...
        # Start background NPV calculation
        npv_task = create_task(self._calculate_npv_batch(customer_id, quick_results, configuration))
    ...
    async def _calculate_npv_batch(
        self, customer_id: str, cars: List[Dict], configuration: Dict[str, Any]
    ):
        ...
        for batch in batches:
            task = loop.run_in_executor(
                self._executor, self._calculate_npv_for_batch, customer, batch, configuration
            )
    ...
    def _calculate_npv_for_batch(
        self, customer: Dict, cars: List[Dict], configuration: Dict[str, Any]
    ):
    ...
    def _quick_search(self, search_term: str, limit: int) -> List[Dict]:
        ...
        for car in all_inventory:
            if (
                search_lower in car.get("brand", "").lower()
                or search_lower in car.get("model", "").lower()
                or search_lower in str(car.get("year", ""))
            ):
                results.append(car)
    ...
    async def get_search_suggestions(self, customer_id: str, partial_query: str) -> List[str]:
    ...
    async def universal_search(self, query: str, limit: int = 50) -> Dict[str, Any]:
        """Truly async universal search across all entities"""
        loop = asyncio.get_event_loop()

        # Search in parallel
        tasks = [
            loop.run_in_executor(self._executor, database.search_customers, query, 10, 0),
            loop.run_in_executor(self._executor, self._quick_search, query, 10),
        ]
    ...
    async def get_inventory_filters(self) -> Dict[str, Any]:
        ...
        loop = asyncio.get_event_loop()
        filters = await loop.run_in_executor(self._executor, self._calculate_inventory_filters)
    ...
    def _calculate_inventory_filters(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "km_range": {"min": 0, "max": max(kms) if kms else 200000},
        }
