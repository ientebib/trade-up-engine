...
from app.constants import (
    ...
    UPGRADE_TIER_MIN,
    UPGRADE_TIER_MAX,
    MAX_UPGRADE_TIER_MIN,
    MAX_UPGRADE_TIER_MAX,
    TERM_60_RATE_ADJUSTMENT,
    TERM_72_RATE_ADJUSTMENT,
...
# Limit number of cars processed in parallel to avoid memory spikes
BATCH_SIZE = 50
...
class BasicMatcher:
    ...
    def find_all_viable(
        self, customer: Dict, inventory: List[Dict], custom_fees: Optional[Dict] = None
    ) -> Dict:
        """
        ...
        if (
            custom_fees
            and "interest_rate" in custom_fees
            and custom_fees["interest_rate"] is not None
        ):
        ...
        eligible_cars = [
            car for car in inventory if car["car_price"] > customer["current_car_price"]
        ]
        ...
        # Process cars in smaller batches to limit the number of concurrent
        # futures and keep memory usage predictable
        for i in range(0, len(eligible_cars), BATCH_SIZE):
            batch = eligible_cars[i : i + BATCH_SIZE]
            tasks = []
            for car in batch:
            ...
            # Explicitly drop references to completed futures
            tasks.clear()

        organized = {"Refresh": [], "Upgrade": [], "Max Upgrade": []}
