...
async def lifespan(app: FastAPI):
    ...
    app.state.customers_df = pd.DataFrame()
    app.state.inventory_df = pd.DataFrame()
    ...
    try:
        ...
        app.state.customers_df = customers_df
        app.state.inventory_df = inventory_df
...
def calculate_real_metrics():
    ...
    customers_df = app.state.customers_df
...
def run_scenario_analysis(config: ScenarioConfig):
    ...
    """
    return core_run_scenario_analysis(
        config.dict(), app.state.customers_df, app.state.inventory_df
    )
